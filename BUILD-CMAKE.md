...
| `0008` | `ssh_packet_write_poll()` writes its output buffer through `w32_write_sshbuf()` (`win32sendbuf.c`) rather than `write()`: one line, and the buffer is handed over rather than copied. Send ~1.4 → ~1.5 GB/s wall-clock. |
| `0009` | `wait_for_any_event()`, the one alertable wait everything in the port blocks in, calls `pumps_flush_before_wait()` first: whatever the writer pumps hold goes out before the main thread goes idle, which is what lets the pumps be woken in batches the rest of the time. |
